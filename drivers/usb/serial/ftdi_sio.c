...
static void ftdi_determine_type(struct usb_serial_port *port)
{
	...
	priv->baud_base = 48000000 / 2;   //Tbao ????
						...
						unsigned char status[2])
{
	...
	dev_dbg(&port->dev, "%s - 0x%02x%02x\n", __func__, status[0],
								status[1]);
	printk("#########%s,%s,%d\n",__FILE__,__func__,__LINE__);  //Tbao
out:
	kfree(buf);
...
static int ftdi_tiocmget(struct tty_struct *tty)
{
	...
	printk(KERN_INFO "#########ret: %d  ; getcmget buf[0]: 0x%x\n",ret, buf[0]);  //Tbao
	printk(KERN_ALERT "#########ret: %d  ; getcmget buf[0]: 0x%x\n",ret, buf[0]);
	return ret;
}
